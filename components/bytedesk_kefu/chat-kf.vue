<template>
	<view>
		<uni-notice-bar :backgroundColor="{backgroundColor: '#FFF'}" :show-close="true" :show-icon="true" :single="true"
			:scrollable="true" v-if="showTopTip" :text="topTip" @close="closeTopTip" back-icon-color="#000"
			title-color="#000" />
		...
		<view class="content" @touchstart="hideDrawer">
			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation"
				:scroll-top="scrollTop" :scroll-into-view="scrollToView" @scrolltoupper="loadMoreMessages"
				upper-threshold="50">
				<!-- 加载历史数据 waitingUI -->
				...
				<view class="row" v-for="(message, index) in messages" :key="index" :id="'msg'+message.mid">
					...
					<view v-if="is_type_commodity(message)" id="goods" class="goods-info"
						@click="commodityCallback(message)">
						<view class="goods-pic">
							<image id="goods-pic" alt="" style="width: 100rpx; height: 100rpx;" width="50px"
								height="50px" :src="jsonObject(message.content).imageUrl"></image>
						</view>
						<view class="goods-desc">
							<view id="goods-name" class="goods-name">{{ jsonObject(message.content).title }}</view>
							<view class="goods-more">
								<view id="goods-price" class="goods-price">￥{{ jsonObject(message.content).price }}
								</view>
								<view id="goods-sendlink" class="goods-sendlink" @click="sendCommodityMessageSync()">
								</view>
							</view>
						</view>
					</view>
					...
					<view class="system">
						...
						<view class="text" v-else-if="is_type_notification_visitor_close(message)">{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_auto_close(message)">{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_thread_reentry(message)">
							{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_offline(message)">{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_invite_rate(message)">{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_rate_result(message)">{{ message.content }}
						</view>
						<view class="text" v-else-if="is_type_notification_rate_helpful(message)">{{ message.content }}
						</view>
					...
					<block v-if="!is_type_notification(message) && !is_type_commodity(message)">
						...
						<view class="my" v-if="is_self(message)">
							...
							<view class="left">
								...
								<view class="status">
									...
									<view v-if="is_sending(message)" class="fa fa-spinner fa-spin"
										style="font-size:12px">发送中</view>
									<view v-else-if="is_stored(message)" class="fa fa-times-circle"
										style="font-size:10px"></view>
									<view v-else-if="is_received(message)" style="font-size:10px; margin-right: 5px;">送达
									</view>
									<view v-else-if="is_read(message)" style="font-size:10px; margin-right: 5px;">已读
									</view>
									<view v-else-if="is_error(message)" class="fa fa-times-circle"
										style="font-size:12px" @click="resendButtonClicked(message)">失败</view>
								</view>
								<!-- 文字消息 -->
								...
								<!-- 语言消息 -->
								<view v-if="message.type=='voice'" class="bubble voice" @tap="playVoice(message)"
									:class="playMsgid == message.mid ? 'play' : ''">
									<view class="length">{{message.length}}</view>
									<view class="icon my-voice"></view>
								</view>
								<!-- 图片消息 -->
								<view v-if="is_type_image(message)" class="bubble img"
									@tap="previewImageMessage(message)">
						...
						<view class="other" v-if="!is_self(message)">
							...
							<view class="right">
								...
								<!-- 语音消息 -->
								<view v-if="is_type_voice(message)" class="bubble voice" @tap="playVoice(message)"
									:class="playMsgid === message.mid ? 'play': ''">
								...
								<view v-if="is_type_robot(message)" class="bubble">
									...
									<view class="flex-column-start" style="color: #2fa39b;">
										<rich-text :nodes="message.content"
											style="color: black;font-size: 25rpx; margin-top:20rpx;margin-bottom:10rpx;">
										</rich-text>
										<view class="flex-row-start  padding-top-sm"
											v-for="(item, index) in message.answers" :key="index">
											<text @click="queryAnswer(item)" style="margin-top: 20rpx;">
												{{ item.question }}
											</text>
										</view>
								...
								<view v-if="is_type_robot_v2(message)" class="bubble">
									<view class="flex-column-start" style="color: #2fa39b;">
										...
										<view class="flex-row-start  padding-top-sm"
											v-for="(item, index) in message.categories" :key="index">
		...
		<view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="textbox">
				...
				<view class="text-mode" :class="isVoice?'hidden':''">
					<view class="box">
						<textarea auto-height="true" v-model="inputContent" @input="onInputChange"
							@focus="textareaFocus" />
					</view>
...
<script>
	...
	export default {
			...
			replaceFace(content) {
				...
				if (matchresult) {
					for (var i = 0; i < matchresult.length; i++) {
						result = result.replace(matchresult[i],
							'<img height=\'25px\' width=\'25px\' style=\'margin-bottom:4px;\' src=\'' + this
							.emotionBaseUrl + emotionMap[matchresult[i]] + '\'>')
					}
				}
				return result
			},
			...
			requestThread() {
				...
				if (this.option.v2robot === undefined || this.option.v2robot === null || this.option.v2robot === false) {
					httpApi.requestThread(this.option.wid, this.option.type, this.option.aid, function(response) {
						console.log('request thread success', app.option.wid, app.option.type, app.option.aid,
							response)
						//
		...
		// 		// 显示留言界面
			...
			switchLeaveMessage() {
				uni.redirectTo({
					url: '../../components/bytedesk_kefu/leavemsg?wid=' + this.option.wid + '&type=' + this.option
						.type + '&aid=' + this.option.aid + '&tip=' + this.leaveMessageTip
				});
			},
			// 本地显示商品信息
			appendCommodityInfo() {
			...
			onMessageReceived(messageObject) {
				...
				if ((messageObject.type === 'text' ||
					...
					if (messageObject.user.uid !== this.uid && messageObject.type !== 'robot' && messageObject.type !==
						"robot_result") {
						// 发送已读回执
