<template>
	<scroll-view scroll-y="true" style="height: 100vh;background-color:#3A3842;">
		<view class="step">
			...
			<image v-if="state==2||state==99" :src="IMAGE_URL+'/wxapp/withdraw/ic_withdrawal_goto_red.png'"
				class="gotoImage" mode="widthFix"></image>
			<image v-else :src="IMAGE_URL+'/wxapp/withdraw/ic_withdrawal_goto.png'" class="gotoImage" mode="widthFix">
			</image>

			<view class="flex-sub item">
				<image v-if="state==2" :src="IMAGE_URL+'/wxapp/withdraw/ic_withdrawal_step5_success.png'" class="image"
					mode="widthFix"></image>
				<image v-if="state==99" :src="IMAGE_URL+'/wxapp/withdraw/ic_withdrawal_step4_fail.png'" class="image"
					mode="widthFix"></image>
				<image v-if='state==1' :src="IMAGE_URL+'/wxapp/withdraw/ic_withdrawal_step4.png'" class="image" mode="widthFix">
				</image>

				<text v-if="state==1" class="text" style="color: #cdcdcd;">充值成功</text>
				<text v-else class="text">充值成功</text>
			</view>
		...
		<view
			...
			<view style="display: flex;">
				...
				<text style="font-size: 28rpx;color: #D5101A; margin-top: 5rpx;">¥{{amount}}</text>
			</view>
			...
			<view style="display: flex;margin-top: 40rpx;">
				...
				<text style="font-size: 28rpx;color: #333;margin-top: 5rpx;">{{getText(time)}}</text>
			</view>
			<view style="display: flex;margin-top: 40rpx;" v-if="applyTime!=''">
				<text style="font-size: 28rpx;color: #333;min-width: 150rpx;">审核时间</text>
				<text style="font-size: 28rpx;color: #333;margin-top: 5rpx;">{{getText(applyTime)}}</text>
			</view>
			<view style="display: flex;margin-top: 40rpx;" v-if="state==99">
				<text style="font-size: 28rpx;color: #333;min-width: 150rpx;">驳回理由</text>
				<text style="font-size: 28rpx;color: #333;margin-top: 5rpx;">{{reason}}</text>
			</view>
		</view>
	...
	export default {
		...
		onLoad(option) {
			...
			if (option != null) {
				...
				if (option.applyTime != null) {
					this.applyTime = option.applyTime
