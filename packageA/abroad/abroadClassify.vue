	...
	<view class="u-wrap">
		...
		<view class="u-menu-wrap">
			<scroll-view scroll-y scroll-with-animation class="u-tab-view menu-scroll-view" :scroll-top="scrollTop">
				<view v-for="(item,index) in countryCategoryList" :key="index" class="u-tab-item" :class="[current==index ? 'u-tab-item-active' : '']"
				 :data-current="index" @tap.stop="swichMenu(index)">
					<text class="u-line-1">{{item.first.name}}</text>
				</view>
			</scroll-view>
...
<script>
	export default {
		...
		onLoad(options) {
			this.getCountryCategoryList(options.id)
			this.country_id = options.id
			this.country_icon = options.icon
			uni.setNavigationBarTitle({title:options.name})
		},
		methods: {
			// 一级分类列表
			getCountryCategoryList(id) {
				this.$u.post('/api/v2/app/abroad/category_list',{country_id:Number(id)}).then(res => {
					if (res.data.code == "FAIL") {
						this.$u.toast(res.data.msg);
						return
					}
					this.countryCategoryList = res.data.data
				})
			},
	
			// toSearch() {
			// 	this.$u.post('/api/v2/app/abroad/search_country').then(res => {
