	...
	<view>
		...
		<scroll-view scroll-y="true" class="scroll">
			<view class="goods-box">
				<goods-list :goodsList="goodsList" :isHot="true" :hideShareBtn="roleLevel==500" @shareBtn='shareBtn'>
				</goods-list>
			</view>
		</scroll-view>
		<!-- 分享弹出框 -->
...
<script>
	export default {
		...
		methods: {
			...
			getList(url) {
				// 高佣特推 特惠专区
				if (this.type == 'gaoyong' || this.type == 'tehui') {
					let data = {status:(this.type == 'gaoyong'?1:2)}
					console.log(data)
					this.$u.post('api/v1/goods/recommend/list',data).then(res => {
						console.log(res);
